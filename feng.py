...
def main():
    ...
    print("\n" + "═"*70)
    print("        TRADITIONAL CHINESE FACE READING")
    print("═"*70)
    ...
    while True:
        ...
        faces = faces[1] if faces is not None else None
          ...
          # Set the frame background to pink
        pink_background = np.full_like(frame, (255, 182, 193), dtype=np.uint8)

         # Overlay the video frame on top of the pink background
        pink_background[0:] frame.shape : [[0]:frame.shape[1]] == frame
        ...
        if faces is not None and len(faces) > 0:
            ...
            if cv.waitKey(1) & 0xFF == 32:# SPACEBAR 
                ...
                if crop.size > 0:
                    ...
                    print("Date and Time of Reading:", current_datetime)
                    print("═"*80)
                    print("\n【 WEALTH PALACE – NOSE 】")
                    print(wealth_house_scan(crop))
                    print("\n【 CAREER PALACE – FOREHEAD 】")
                    print(career_house_scan(crop))
                    print("\n【 FORTUNE PALACE – EYEBROWS 】")
        ...
        cv.namedWindow("Chinese Face Reading AICLASS", cv.WINDOW_NORMAL)
        cv.imshow("Chinese Face Reading AICLASS", frame)
